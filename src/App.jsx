...
export default function App() {
  // Page View state: "landing" | "builder" | "admin"
  ...
  // If in Admin Panel, render AdminPanel view
  if (view === "admin") {
    return (
      <AdminPanel
        onBackToBuilder={() => navigateToBuilder()}
    ...
    >
      ...
      <header className="no-print h-16 border-b border-white/[0.08] bg-zinc-950/75 backdrop-blur-xl sticky top-0 z-30 px-4 sm:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Admin Panel Quick Link Button - Visible only for authenticated Admins */}
          {user?.role === "admin" && (
            <button
              type="button"
